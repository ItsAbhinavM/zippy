...
function EmptyState() {
  return (
    <Paper variant="outlined" sx={{ p: 4, borderStyle: "dashed", borderColor: "divider", textAlign: "center", bgcolor: "transparent" }}>
...
export function CardGrid() {
  const hasAnything = useFinancialStore((s) => {
    ...
    if (!st) return false;
    return (
      st.income.length > 0 || st.essential_expenses.length > 0 ||
      st.optional_expenses.length > 0 || st.payments.length > 0 ||
      st.missing_fields.length > 0 || st.conflicts.length > 0
    );
  });
